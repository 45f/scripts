...
public class EntityInteractor extends Interactor<Entity> {
	...
	public EntityInteractor setEntity(Entity e) {
		target = e;
		rect = GraphicUtilities.getModelBoundingBox(getBot(), target.getGridX(), target.getGridY(), target.getZ(),
				target.getModel());
		return this;
	}
	...
	public boolean interact(String interaction, boolean sleep, int deviate) throws InterruptedException {
		if (item == null && (target == null || !target.hasAction(interaction)))
			return false;

		if (getMenuAPI().isOpen()) {
			getMouse().click(false);
			if (sleep)
				sleep(rand(1 * deviate, 10 * deviate));
		}
		debug("Interacting");

		if (getMap().distance(target.getPosition()) > 6 && !getLocalWalker().walk(target.getPosition()))
			return false;

		if (item != null) {
			debug("Selecting item");
			if (getInventory().isItemSelected()
					&& !getInventory().getSelectedItemName().equalsIgnoreCase(item.getName())) {
				getInventory().deselectItem();
				if (sleep)
					sleep(rand(10 * deviate, 25 * deviate));
			}
			if (!item.hover())
				return false;
			debug("Hovered");
			if (sleep)
				sleep(rand(1 * deviate, 5 * deviate));
			if (!getMouse().click(false))
				return false;
			debug("Clicked");
			interaction = "Use " + item.getName() + " -> " + target.getName();
		}
		while (!getMouse().isOnCursor(target.getRaw()))
			target.hover();
		...
		if (getMenuAPI().getTooltip().startsWith(interaction) && getMenuAPI().getTooltip().endsWith(target.getName()))
			return getMouse().click(false);
		if (!getMouse().click(true))
			return false;

		if (sleep)
			sleep(rand(25 * deviate, 40 * deviate));
		...
		for (int i = 0; i < options.size(); i++) {
			...
			String s = o.action;
			debug(o.name);
			if ((item != null && getMenuAPI().stripFormatting(o.name).endsWith(target.getName()))
					|| (s.equalsIgnoreCase(interaction)
							&& getMenuAPI().stripFormatting(o.name).startsWith(target.getName()))) {
				index = i;
				break;
			}
		}
		...
		if (getMouse().move(finalX, finalY))
			return false;
		if (sleep)
			sleep(rand(5 * deviate, 10 * deviate));
		return getMouse().click(false);
	}
}
